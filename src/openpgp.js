...
/**
 * Decrypts session key for message
 * @param  {module:key~Key}                privateKey private key with decrypted secret key data
 * @param  {module:message~Message|String} msg    the message object or ASCII armored text of the encrypted data
 * @param  {String} password               (optional) password used to decrypt
 * @return {Promise<(String|null)>}        decrypted session key as as native JavaScript string
 *                                         or null if no literal data found
 * @static
 */
function decryptSessionKey(privateKey, msg, password) {
  //if (asyncProxy) {
  //  return asyncProxy.decryptMessage(privateKey, msg, format);
  //}
  ...
  return execute(function() {
    if(typeof msg == 'String')
      msg = message.readArmored(msg);
    return msg.decryptSessionKey(privateKey, password);
